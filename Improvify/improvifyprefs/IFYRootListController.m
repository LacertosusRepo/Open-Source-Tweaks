...
@implementation IFYRootListController
	...
	-(void)colorPicker {
		...
		UIColor * initialColor = LCPParseColorString(buttonColor, @"#1DB954");
...
@implementation ImprovifyMiscellaneousSettingsViewController

	-(NSArray *)specifiers {
		if (!_specifiers) {
			_specifiers = [[self loadSpecifiersFromPlistName:@"ImprovifyMiscellaneousSettingsViewController" target:self] retain];
		}
		return _specifiers;
	}
