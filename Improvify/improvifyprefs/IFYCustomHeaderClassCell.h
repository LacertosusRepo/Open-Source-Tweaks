...
@implementation IFYCustomHeaderCell
@synthesize iconView, headerLabel, subHeaderLabel;
	...
	-(id)init {
		...
		if(self) {
			...
			self.iconView = [[UIImageView alloc] initWithImage:[[UIImage alloc] initWithContentsOfFile:@"/Library/PreferenceBundles/improvifyprefs.bundle/iconlarge.png"]];
      ...
      [self.headerLabel setText:@"Improvify"];
	...
	-(NSArray *)randomLabels {
		return @[@"Lets make Spotify better, together", @"No spooky piracy here OwO", @"Open source on Github!"];
	}
